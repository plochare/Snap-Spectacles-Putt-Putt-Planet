...
(function (CatmullRomSpline) {
     ...
     */
    function generateSpline(controlPoints, resolution) {
        ...
        for (let i = 0; i < controlPoints.length - 1; i++) {
            ...
            for (let j = 0; j < resolution; j++) { // <=
                ...
                const forward = new vec3(tangent.x - position.x, tangent.y - position.y, tangent.z - position.z).normalize();
                const up = vec3.up();
    ...
    CatmullRomSpline.generateSpline = generateSpline;
    /**
     ...
     */
    function worldToSplineSpace(position, splinePoints) {
    ...
    CatmullRomSpline.worldToSplineSpace = worldToSplineSpace;
    /**
     ...
     */
    function splineSpaceToWorld(t, splinePoints) {
        if (splinePoints.length < 2) {
        ...
        const position = new vec3(p1.position.x + (p2.position.x - p1.position.x) * localT, p1.position.y + (p2.position.y - p1.position.y) * localT, p1.position.z + (p2.position.z - p1.position.z) * localT);
        const rotation = new quat(p1.rotation.x + (p2.rotation.x - p1.rotation.x) * localT, p1.rotation.y + (p2.rotation.y - p1.rotation.y) * localT, p1.rotation.z + (p2.rotation.z - p1.rotation.z) * localT, p1.rotation.w + (p2.rotation.w - p1.rotation.w) * localT);
        return { position, rotation };
    }
    CatmullRomSpline.splineSpaceToWorld = splineSpaceToWorld;
})(CatmullRomSpline || (exports.CatmullRomSpline = CatmullRomSpline = {}));
//# sourceMappingURL=CatmullRomSpline.js.map