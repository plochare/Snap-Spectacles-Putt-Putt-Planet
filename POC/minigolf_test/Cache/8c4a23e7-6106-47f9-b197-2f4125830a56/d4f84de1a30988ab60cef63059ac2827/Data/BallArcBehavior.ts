...
export class BallArcBehavior extends BaseScriptComponent {
  ...
  private calculateMidpoint(pos1: vec3, pos2: vec3): vec3 {
    ...
    /**
     * Ball Collision Check if it's floor bounce
     * */
    public collisionBall(){

        if (this.ballObject.getTransform().getLocalPosition().y < 1.0){
            this.bounceCounter++
            if (this.bounceCounter>3){
                this.resetBall()
            }
    ...
    * */
    setRandomBallArc(){
        ...
        let randomDistance = Math.floor(Math.random() * (60 - 14 + 1)) + 14;
        // 
