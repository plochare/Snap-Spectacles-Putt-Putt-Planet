...
export class BallArcBehavior extends BaseScriptComponent {
    ...
    */
    ...
    private calculateDistance(pos1: vec3, pos2: vec3): vec3 {
       return pos1.add(pos2).uniformScale(0.5)
    }
    ...
    displayStroke(){
        ...
        const dragDistz = dragpoint.z - startpoint.z
        const dragDistx = dragpoint.x - startpoint.x
        /*
        const sumOfSquares = Math.pow(this.dragDistz, 2) + Math.pow(this.dragDistx, 2) 

        const dragDistance = Math.sqrt(this.sumOfSquares)

        const launchAngle = (this.calculateAngle(this.dragObject.getTransform().getWorldPosition(), this.t1Object.getTransform().getWorldPosition()) + 90) * -0.0174533
        
        this.t8Object.getTransform().setLocalRotation(quat.quatFromEuler(0,this.launchAngle,0))
        
        this.t8Object.getTransform().setWorldPosition(new vec3(this.startpoint.getTransform().getWorldPosition().x + this.dragDistance*Math.sin(this.launchAngle), 0, this.startpoint.getTransform().getWorldPosition().z + this.dragDistance*Math.cos(this.launchAngle)))
        */
        
        this.t8Object.getTransform().setWorldPosition(new vec3(this.dragObject.getTransform().getWorldPosition().x, this.dragObject.getTransform().getWorldPosition().y, this.dragObject.getTransform().getWorldPosition().z))
        //this.t1Object.getTransform().setLocalPosition(new vec3(startpoint.x + 0.2*dragDistance, 0, startpoint.z + 0.2*dragHeight))
        //this.t2Object.getTransform().setLocalPosition(new vec3(startpoint.x + 0.3*dragDistance, 0, startpoint.z + 0.3*dragHeight))
        //this.t3Object.getTransform().setLocalPosition(new vec3(startpoint.x + 0.4*dragDistance, 0, startpoint.z + 0.4*dragHeight))
        //this.t4Object.getTransform().setLocalPosition(new vec3(startpoint.x + 0.5*dragDistance, 0, startpoint.z + 0.5*dragHeight))
        //this.t5Object.getTransform().setLocalPosition(new vec3(startpoint.x + 0.6*dragDistance, 0, startpoint.z + 0.6*dragHeight))
        //this.t6Object.getTransform().setLocalPosition(new vec3(startpoint.x + 0.7*dragDistance, 0, startpoint.z + 0.7*dragHeight))
        //this.t7Object.getTransform().setLocalPosition(new vec3(startpoint.x + 0.8*dragDistance, 0, startpoint.z + 0.9*dragHeight))
        //this.t8Object.getTransform().setLocalPosition(new vec3(startpoint.x + 0.9*dragDistance, 0, startpoint.z + 0.9*dragHeight))
        //
